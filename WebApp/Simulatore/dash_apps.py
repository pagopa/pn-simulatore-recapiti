...
def update_linechart_regioni_recap(regioni_sel, recap_sel, pathname):
    ...
    prov_recap_5_sett = view_tabella_sintesi_reg_recap.objects.filter(SIMULAZIONE_ID = id_simulazione).order_by(F('SUM_COUNT_REQUEST_PRIMA_SETT_MESE_SUCC').desc(nulls_last=True)).values("PROVINCIA_RECAPITISTA")
    ...
    fig_line_reg_recap.add_annotation(
        ...
        yref="paper",      
        text="Soglia oltre la 1° sett.<br>del mese successivo",
        showarrow=False,
...
app_confronto.layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.H2(id="titolo-simulazione-1",style={"text-align":"center"})
        ], className="col-sm"),
        html.Div([
            html.H2(id="titolo-simulazione-2",style={"text-align":"center"})
        ], className="col-sm")
...
@app_confronto.callback(
    ...
    Input("url", "pathname"),
    Input("sorting-filter-1","value"),
    Input("selecting-filter-1","value")
)
def populate_dropdown_ente_1(pathname, value, value_sel):
    from .models import view_tabella_sintesi_ente
    id_simulazione = int(pathname.strip("/").split("/")[-2])
    sorting_value = value
    if ("Postalizzazioni" in sorting_value):
...
@app_confronto.callback(
    ...
    Input("url", "pathname"),
    Input("sorting-filter-2","value"),
    Input("selecting-filter-2","value")
)
def populate_dropdown_ente_2(pathname, value, value_sel):
    from .models import view_tabella_sintesi_ente
    id_simulazione = int(pathname.strip("/").split("/")[-1])
    sorting_value = value
    if ("Postalizzazioni" in sorting_value):
...
@app_confronto.callback(
    Output("datatable-enti-ag-1", "rowData"),
    Output("datatable-enti-ag-1", "columnDefs"),
    Input("url", "pathname"),
    Input("sorting-filter-1","value")
)
def update_table_ente_ag(pathname, value):
...
@app_confronto.callback(
    Output("datatable-enti-ag-1", "exportDataAsCsv"),
    Input("btn-export-enti-1", "n_clicks"),
    prevent_initial_call=True,
)
def export_data_enti(n_clicks):
    if n_clicks > 0:
...
@app_confronto.callback(
    Output("datatable-enti-ag-2", "rowData"),
    Output("datatable-enti-ag-2", "columnDefs"),
    Input("url", "pathname"),
    Input("sorting-filter-2","value")
)
def update_table_ente_ag(pathname, value):
...
@app_confronto.callback(
    Output("datatable-enti-ag-2", "exportDataAsCsv"),
    Input("btn-export-enti-2", "n_clicks"),
    prevent_initial_call=True,
)
def export_data_enti(n_clicks):
    if n_clicks > 0:
...
@app_confronto.callback(
    Output("line-reg-plot-1", "figure"),
    Input("regione-filter-1", "value"),
    Input("recap-filter-1", "value"),
    Input("url", "pathname")
)
def update_linechart_regioni_recap_1(regioni_sel, recap_sel, pathname):
    from .models import table_output_grafico_reg_recap
    from .models import view_tabella_sintesi_reg_recap
    id_simulazione = int(pathname.strip("/").split("/")[-2])
    if isinstance(regioni_sel, str):
...
@app_confronto.callback(
    Output("line-reg-plot-2", "figure"),
    Input("regione-filter-2", "value"),
    Input("recap-filter-2", "value"),
    Input("url", "pathname")
)
def update_linechart_regioni_recap_2(regioni_sel, recap_sel, pathname):
    from .models import table_output_grafico_reg_recap
    from .models import view_tabella_sintesi_reg_recap
    id_simulazione = int(pathname.strip("/").split("/")[-1])
    if isinstance(regioni_sel, str):
...
@app_confronto.callback(
    Output("datatable-reg-recap-ag-1", "rowData"),
    Output("datatable-reg-recap-ag-1", "columnDefs"),
    Input("regione-filter-1", "value"),
    Input("recap-filter-1", "value"),
    Input("url", "pathname")
)
def update_table_reg_recap_ag_1(regioni_sel, recap_sel, pathname):
    from .models import view_tabella_sintesi_reg_recap
    id_simulazione = int(pathname.strip("/").split("/")[-2])
    tab_sintesi_reg_recap = view_tabella_sintesi_reg_recap.objects.filter(SIMULAZIONE_ID = id_simulazione, REGIONE__in = regioni_sel, UNIFIED_DELIVERY_DRIVER__in = recap_sel).order_by(F('SUM_COUNT_REQUEST_PRIMA_SETT_MESE_SUCC').desc(nulls_last=True)).values()
    df_tab_sintesi_reg_recap = pd.DataFrame(tab_sintesi_reg_recap)
...
@app_confronto.callback(
    Output("datatable-reg-recap-ag-1", "exportDataAsCsv"),
    Input("btn-export-reg-recap-1", "n_clicks"),
    prevent_initial_call=True,
)
def export_data_reg_recap(n_clicks):
    if n_clicks > 0:
...
@app_confronto.callback(
    Output("datatable-reg-recap-ag-2", "rowData"),
    Output("datatable-reg-recap-ag-2", "columnDefs"),
    Input("regione-filter-2", "value"),
    Input("recap-filter-2", "value"),
    Input("url", "pathname")
)
def update_table_reg_recap_ag_2(regioni_sel, recap_sel, pathname):
    from .models import view_tabella_sintesi_reg_recap
    id_simulazione = int(pathname.strip("/").split("/")[-1])
    tab_sintesi_reg_recap = view_tabella_sintesi_reg_recap.objects.filter(SIMULAZIONE_ID = id_simulazione, REGIONE__in = regioni_sel, UNIFIED_DELIVERY_DRIVER__in = recap_sel).order_by(F('SUM_COUNT_REQUEST_PRIMA_SETT_MESE_SUCC').desc(nulls_last=True)).values()
    df_tab_sintesi_reg_recap = pd.DataFrame(tab_sintesi_reg_recap)
...
@app_confronto.callback(
    Output("datatable-reg-recap-ag-2", "exportDataAsCsv"),
    Input("btn-export-reg-recap-2", "n_clicks"),
    prevent_initial_call=True,
)
def export_data_reg_recap(n_clicks):
    if n_clicks > 0:
...
def update_chart_recap_1(pathname):
    from .models import table_output_grafico_reg_recap
    id_simulazione = int(pathname.strip("/").split("/")[-2])
    recap = table_output_grafico_reg_recap.objects.filter(SIMULAZIONE_ID = id_simulazione).values()
    df_recap = pd.DataFrame(recap)
...
def update_chart_recap_2(pathname):
    from .models import table_output_grafico_reg_recap
    id_simulazione = int(pathname.strip("/").split("/")[-1])
    recap = table_output_grafico_reg_recap.objects.filter(SIMULAZIONE_ID = id_simulazione).values()
    df_recap = pd.DataFrame(recap)
...
@app_confronto.callback(
    Output("datatable-region-ag-1", "rowData"),
    Output("datatable-region-ag-1", "columnDefs"),
    Input("recap-only-filter-1", "value"),
    Input("url", "pathname")
)
...
@app_confronto.callback(
    Output("datatable-region-ag-1", "exportDataAsCsv"),
    Input("btn-export-picco-1", "n_clicks"),
    prevent_initial_call=True,
)
def export_data(n_clicks):
    if n_clicks > 0:
...
@app_confronto.callback(
    Output("datatable-region-ag-2", "rowData"),
    Output("datatable-region-ag-2", "columnDefs"),
    Input("recap-only-filter-2", "value"),
    Input("url", "pathname")
)
...
@app_confronto.callback(
    Output("datatable-region-ag-2", "exportDataAsCsv"),
    Input("btn-export-picco-2", "n_clicks"),
    prevent_initial_call=True,
)
def export_data(n_clicks):
    if n_clicks > 0:
