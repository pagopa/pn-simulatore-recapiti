...
class view_output_capacity_setting_mock(pg.View):
    ...
    sql = """
        ...
        "SUM_SENDERLIMIT_MOCK_BY_MONTH"."ID_SIMULAZIONE",
        "PROV_REG"."REGIONE",
        "PROV_REG"."COD_SIGLA_PROVINCIA",
        "PROV_REG"."PROVINCIA",
        ...
        public."DECLARED_CAPACITY"."PRODUCT_890",
        public."DECLARED_CAPACITY"."PRODUCT_AR",
        ("SUM_SENDERLIMIT_MOCK_BY_MONTH"."DELIVERY_DATE" + ((1 - EXTRACT(DOW FROM  "SUM_SENDERLIMIT_MOCK_BY_MONTH"."DELIVERY_DATE")::int + 7) % 7) * INTERVAL '1 day') AS "TEST A",
        (("SUM_SENDERLIMIT_MOCK_BY_MONTH"."DELIVERY_DATE" + INTERVAL '1 month') + ((1 - EXTRACT(DOW FROM "SUM_SENDERLIMIT_MOCK_BY_MONTH"."DELIVERY_DATE" + INTERVAL '1 month')::int + 7) % 7) * INTERVAL '1 day') AS "TEST B",
        ...
        WHERE (
        public."DECLARED_CAPACITY"."ACTIVATION_DATE_FROM" >= 
        ("SUM_SENDERLIMIT_MOCK_BY_MONTH"."DELIVERY_DATE" + 
            ((1 - EXTRACT(DOW FROM  "SUM_SENDERLIMIT_MOCK_BY_MONTH"."DELIVERY_DATE")::int + 7) % 7) 
            * INTERVAL '1 day')
        AND 
        public."DECLARED_CAPACITY"."ACTIVATION_DATE_FROM" <= 
        ...
        "SIMULAZIONI_MOCK" AS (
        SELECT DISTINCT "ID_SIMULAZIONE", "DELIVERY_DATE"
        FROM public."SENDER_LIMIT_MOCK"
        ),
        ...
        ON "FILTERED_CAPACITY_BY_PRODUCT"."ACTIVATION_DATE_FROM" >= 
            ("SIMULAZIONI_MOCK"."DELIVERY_DATE" + 
            ((1 - EXTRACT(DOW FROM  "SIMULAZIONI_MOCK"."DELIVERY_DATE")::int + 7) % 7) 
            * INTERVAL '1 day')
            AND 
            "FILTERED_CAPACITY_BY_PRODUCT"."ACTIVATION_DATE_FROM" <= 
            (("SIMULAZIONI_MOCK"."DELIVERY_DATE" + INTERVAL '1 month') + ((1 - EXTRACT(DOW FROM "SIMULAZIONI_MOCK"."DELIVERY_DATE" + INTERVAL '1 month')::int + 7) % 7) * INTERVAL '1 day')
        ), 
        ...
        AND "FILTERED_CAPACITY_BY_PRODUCT_AND_SIMULAZIONE"."ID_SIMULAZIONE" = "FILTERED_CAPACITY_BY_PRODUCT_MOCK"."ID_SIMULAZIONE"
        )
        SELECT
        ROW_NUMBER() OVER () AS id,  
        "UNIFIED_DELIVERY_DRIVER",
        ...
        GROUP BY "UNIFIED_DELIVERY_DRIVER","ID_SIMULAZIONE","COD_SIGLA_PROVINCIA","MONTH_DELIVERY","ACTIVATION_DATE_FROM","ACTIVATION_DATE_TO","CAPACITY","PEAK_CAPACITY","PRODUCTION_CAPACITY","REGIONE","PROVINCIA","PRODUCT_890","PRODUCT_AR"
        """
