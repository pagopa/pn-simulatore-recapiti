...
## @params: [JOB_NAME]
args = getResolvedOptions(sys.argv, ['JOB_NAME','mese_recupero_dati','s3_bucket','secretsManager_SecretId','jdbc_connection'])
# args = getResolvedOptions(sys.argv, ['JOB_NAME'])
# mese_recupero_dati = '2025-10-06'
...
df_senderlim_tot = df_senderlim_tot.drop('ID')\
                                   .filter(F.col('DELIVERY_DATE').cast('string')==mese_recupero_dati)
...
if last_update_timestamp.date() == datetime.now().date():
    ...
    anno = int(mese_recupero_dati[:4])
    mese = int(mese_recupero_dati[5:7])
    print(anno+mese)
    ...
    anno_str = mese_recupero_dati[:4]
    mese_str = mese_recupero_dati[5:7]
